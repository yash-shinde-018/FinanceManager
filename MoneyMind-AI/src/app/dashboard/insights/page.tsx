          ...
          try {
            ...
            if (predictionResult && predictionResult.prediction && predictionResult.prediction > 0) {
              ...
              formattedInsights.push({
                ...
                description: `Based on your spending patterns, AI predicts your next month's spending will be approximately ₹${Math.abs(predValue).toFixed(0).replace(/\B(?=(\d{3})+(?!\d))/g, ',')}. ${lower && upper ? `Expected range: ₹${Math.abs(lower).toFixed(0).replace(/\B(?=(\d{3})+(?!\d))/g, ',')} - ₹${Math.abs(upper).toFixed(0).replace(/\B(?=(\d{3})+(?!\d))/g, ',')}.` : ''} Model accuracy: ${predictionResult.model_accuracy || 'N/A'}`,
                severity: 'info',
                timestamp: new Date(),
                actionRequired: false,
              });
