...
async def generate_insights_from_transactions(transactions: List[Transaction]):
    ...
    try:
        ...
        logger.info(f"Processing {len(df)} transactions for insights")
        ...
        # Generate FORECAST insights using SARIMAX model
        try:
            from spending_forecaster_v2 import ImprovedSpendingForecaster
            forecaster = ImprovedSpendingForecaster()
            ...
            if len(daily_spending) >= 7:  # Need at least 7 days for SARIMAX
                try:
                    forecaster.train_sarimax(daily_spending)
                    
                    # Forecast next 30 days
                    forecast_result = forecaster.sarimax_model.forecast(steps=30)
                    forecast_values = forecast_result.values if hasattr(forecast_result, 'values') else forecast_result
                    
                    next_30_days_predicted = float(forecast_values.sum())
                    avg_predicted_daily = next_30_days_predicted / 30
                    
                    insights.append({
                        "type": "forecast",
                        "message": f"📊 AI predicts you'll spend ₹{next_30_days_predicted:.2f} in the next 30 days (₹{avg_predicted_daily:.2f}/day)",
                        "severity": "low"
                    })
                    
                    # Compare with current spending rate
                    current_daily_avg = df['amount'].mean()
                    if avg_predicted_daily > current_daily_avg * 1.2:
                        insights.append({
                            "type": "forecast",
                            "message": f"⚠️ Forecast shows 20% increase in spending. Your predicted daily spending (₹{avg_predicted_daily:.2f}) is higher than current average (₹{current_daily_avg:.2f})",
                            "severity": "medium"
                        })
                    elif avg_predicted_daily < current_daily_avg * 0.8:
                        insights.append({
                            "type": "forecast",
                            "message": f"✅ Great news! Forecast shows 20% decrease in spending. Keep up the good work!",
                            "severity": "low"
                        })
                except Exception as sarimax_error:
                    logger.warning(f"SARIMAX forecast failed, using simple forecast: {sarimax_error}")
                    # Fallback to simple linear forecast
                    current_daily_avg = df['amount'].mean()
                    next_30_days_predicted = current_daily_avg * 30
                    
                    insights.append({
                        "type": "forecast",
                        "message": f"📊 Based on your current spending pattern, you're projected to spend ₹{next_30_days_predicted:.2f} in the next 30 days (₹{current_daily_avg:.2f}/day)",
                        "severity": "low"
                    })
            else:
                # Not enough data for SARIMAX, use simple average
                if len(df) > 0:
                    current_daily_avg = df['amount'].mean()
                    next_30_days_predicted = current_daily_avg * 30
                    ...
                    insights.append({
                        "type": "forecast",
                        "message": f"📊 Based on your spending pattern, you're projected to spend ₹{next_30_days_predicted:.2f} in the next 30 days (₹{current_daily_avg:.2f}/day). Add more transactions for AI-powered forecasts!",
                        "severity": "low"
                    })
        except Exception as e:
            logger.warning(f"Forecast insight generation failed: {e}")
            # Always provide at least a simple forecast
            if len(df) > 0:
                current_daily_avg = df['amount'].mean()
                next_30_days_predicted = current_daily_avg * 30
                ...
                insights.append({
                    "type": "forecast",
                    "message": f"📊 Based on your average spending, you're projected to spend ₹{next_30_days_predicted:.2f} in the next 30 days",
                    "severity": "low"
                })
        
        # Generate SPENDING ALERT insights (overspending detection)
        logger.info(f"Generating spending alerts from {len(category_totals)} categories")
        # Check if spending is above average in any category
        for cat, amount in category_totals.items():
            percentage = (amount / total * 100) if total > 0 else 0
            
            # Alert if any category is more than 40% of total spending
            if percentage > 40:
                insights.append({
                    "type": "overspending",
                    "message": f"⚠️ High spending alert: {cat} accounts for {percentage:.1f}% of your total spending (₹{amount:.2f}). Consider setting a budget.",
                    "severity": "high"
                })
            # Warning if category is 25-40%
            elif percentage > 25:
                insights.append({
                    "type": "overspending",
                    "message": f"💰 {cat} spending is {percentage:.1f}% of your budget (₹{amount:.2f}). Monitor this category closely.",
                    "severity": "medium"
                })
        
        # Check for recent high spending days
        if len(df) > 0:
            daily_totals = df.groupby('date')['amount'].sum()
            avg_daily_spending = daily_totals.mean()
            std_daily_spending = daily_totals.std()
            
            # Find days with spending > 2 standard deviations above mean
            high_spending_days = daily_totals[daily_totals > (avg_daily_spending + 2 * std_daily_spending)]
            
            if len(high_spending_days) > 0:
                for date, amount in high_spending_days.tail(3).items():
                    insights.append({
                        "type": "overspending",
                        "message": f"📈 Unusual spending detected on {date.strftime('%b %d')}: ₹{amount:.2f} (2x your daily average of ₹{avg_daily_spending:.2f})",
                        "severity": "high"
                    })
        
        logger.info(f"Generated {len(insights)} total insights")
        logger.info(f"Insight types: {[i['type'] for i in insights]}")
